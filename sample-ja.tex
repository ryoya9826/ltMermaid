% ltmermaid.sty の説明と使用例（日本語版）
% コンパイル例: lualatex -shell-escape sample-ja.tex
% 文書クラス: BXjscls の bxjsarticle（LuaLaTeX + ja=standard）
\documentclass[lualatex,luatex,ja=standard,a4paper,11pt]{bxjsarticle}
\usepackage{array}
\usepackage{tabularx}
\usepackage{float}
...
\begin{abstract}
\noindent
\texttt{ltmermaid} パッケージは、\textbf{LuaLaTeX} を使うことにより\LaTeXe コンパイル時に Mermaid 図を同時にレンダリングし、PDFに埋め込みます。Mermaid 図のレンダリングに Mermaid CLI などのツールを利用する必要があるため、シェルエスケープ（\verb|-shell-escape|）が必要です。\texttt{lualatex} を実行する
マシンには、動作する \textbf{Node.js}、\texttt{mmdc}（または \texttt{npx}）、および CLI 用のヘッドレス Chromium が利用できる必要があります。
\end{abstract}
...
Mermaid CLI が環境にない場合、\texttt{ltmermaid} は \texttt{mmdc} などを環境へ自動インストールしません。
Mermaid CLI の標準的なインストールでは、\textbf{Node.js} と \texttt{npm}（または \texttt{npx}）を利用します。
一方、パッケージオプション \texttt{Renderer} に
\texttt{npx -y @mermaid-js/mermaid-cli …} のように \textbf{\texttt{npx -y}} を含むコマンドを指定した場合は、\texttt{npx} の仕様として必要に応じてパッケージの
\textbf{取得・インストール}が行われます（ネットワークアクセスを伴います）。

\texttt{lualatex} 実行時には、\texttt{Renderer} オプションで別コマンドを指さない限り、
既定のレンダラ \texttt{mmdc} が同じ環境から見える必要があります（指す場合はそのコマンドが見えること）。
Mermaid CLI はヘッドレス Chromium（Puppeteer）に依存します。詳細は Mermaid CLI の
ドキュメントを参照してください。
...
\item \textbf{\texttt{Renderer}}:
  レンダラコマンドの接頭辞。省略時は既定で \texttt{mmdc}。
  \texttt{npx -y @mermaid-js/mermaid-cli}などのように明示的に指定することが可能です。
\end{itemize}
\par}
...
\noindent
\textbf{Version 0.2}（2026-04-13）: \texttt{MERMAID\_MMDC} / \texttt{MERMAID\_MMDC\_OPTIONS} を廃止。

\smallskip
\noindent
\textbf{Version 0.1}（2026-04-08）: 初版。
